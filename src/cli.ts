...
import {
  AgentError, ATTESTATION_PATH, TELEMETRY_PATH, VERSION, createEvidenceSubmission,
  createTelemetryPayload, postJson, resolveEndpoint, verifyAttestationReceipt,
} from './core.js';
...
function parseNumber(value: string | undefined, label: string): number {
  if (!value || !/^\d+$/.test(value)) throw new AgentError('INVALID_ARGUMENT', `${label} must be an integer.`);
  return Number(value);
}

async function pushTelemetry(args: string[]): Promise<void> {
...
async function isoMincheck(args: string[]): Promise<void> {
  const parsed = parseArgs({ args, strict: true, allowPositionals: false, options: {
    help: { type: 'boolean', short: 'h' }, endpoint: { type: 'string' }, org: { type: 'string' },
...
export async function run(argv = process.argv.slice(2)): Promise<void> {
  ...
  if (command === 'push-telemetry') return pushTelemetry(args);
  if (command === 'iso-mincheck') return isoMincheck(args);
